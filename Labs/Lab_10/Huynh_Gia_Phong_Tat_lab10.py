...
def start_shutdown_check():
    ...
    while True:
        time.sleep(10)
        nice_print("Checking containers...")
...
def stream_container_logs(container_name):
    ...
    while True:
        try:
            ...
            nice_print(f"Streaming logs for '{container_name}'...")
            ...
            nice_print(f"Retrying log stream for '{container_name}' in 5s...")
            time.sleep(5)

# Startup: monitor containers + start log streamers.
# Everything is in a thread so it doesn't block the main thread.
if __name__ == "__main__":
    nice_print("Starting logging setup and container monitoring...")
